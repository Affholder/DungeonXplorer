...
class AdminController {
    ...
    private function handleImageUpload($fileInputName, $folder, $prefix, $id = null) {
        ...
        $fileName = $prefix . ($id ? '_' . $id : '') . '_' . time() . '.' . $extension;
        $uploadPath = $uploadDir . $fileName;
        
        // Déplacer le fichier uploadé
        if (move_uploaded_file($_FILES[$fileInputName]['tmp_name'], $uploadPath)) {
            // Retourner juste le nom du fichier sans le chemin
            return $fileName;
        } else {
    ...
    // Fonction utilitaire pour supprimer une ancienne image
    private function deleteOldImage($imagePath, $folder) {
        if ($imagePath) {
            // Extraire juste le nom du fichier si c'est un chemin complet
            $fileName = basename($imagePath);
            $fullPath = __DIR__ . '/../../public/images/' . $fileName;
    ...
    public function getChapters() {
        ...
        // Ajouter le chemin complet pour l'affichage
        foreach ($chapters as &$chapter) {
            if (!empty($chapter['image'])) {
                $chapter['image_display'] = '/DungeonXplorer/public/images/' . $chapter['image'];
            } else {
                $chapter['image_display'] = '';
            }
        }
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            try {
                ...
                if (isset($_FILES['chapter_image']) && $_FILES['chapter_image']['error'] === UPLOAD_ERR_OK) {
                    $image = $this->handleImageUpload('chapter_image', 'chapters', 'chapter');
                }
    ...
    public function updateChapter() {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            try {
                ...
                $stmt = $this->db->prepare("SELECT image FROM Chapter WHERE id = ?");
                $stmt->execute([$id]);
                $oldImage = $stmt->fetchColumn();
                
                $image = $oldImage; // Par défaut, on garde l'ancienne image
                
                // Gérer l'upload d'image si présente
                if (isset($_FILES['chapter_image']) && $_FILES['chapter_image']['error'] === UPLOAD_ERR_OK) {
                    $newImage = $this->handleImageUpload('chapter_image', 'chapters', 'chapter', $id);
                    if ($newImage) {
                        $image = $newImage;
                        // Supprimer l'ancienne image
                        $this->deleteOldImage($oldImage, 'chapters');
                    }
                }
    ...
    public function deleteChapter() {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if ($stmt->execute([$id])) {
                ...
                $this->deleteOldImage($image, 'chapters');
    ...
    public function getMonsters() {
        ...
        foreach ($monsters as &$monster) {
            if (!empty($monster['mon_image'])) {
                $monster['image_display'] = '/DungeonXplorer/public/images/' . $monster['mon_image'];
            } else {
                $monster['image_display'] = '';
            }
        }
    ...
    public function addMonster() {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            try {
                ...
                // Gérer l'upload d'image si présente
                if (isset($_FILES['monster_image']) && $_FILES['monster_image']['error'] === UPLOAD_ERR_OK) {
                    $mon_image = $this->handleImageUpload('monster_image', 'monsters', 'monster');
                }
    ...
    public function updateMonster() {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            try {
                ...
                // Récupérer l'ancienne image
                $stmt = $this->db->prepare("SELECT mon_image FROM Monster WHERE id = ?");
                $stmt->execute([$id]);
                $oldImage = $stmt->fetchColumn();
                
                $mon_image = $oldImage; // Par défaut, on garde l'ancienne image
                
                // Gérer l'upload d'image si présente
                if (isset($_FILES['monster_image']) && $_FILES['monster_image']['error'] === UPLOAD_ERR_OK) {
                    $newImage = $this->handleImageUpload('monster_image', 'monsters', 'monster', $id);
                    if ($newImage) {
                        $mon_image = $newImage;
                        // Supprimer l'ancienne image
                        $this->deleteOldImage($oldImage, 'monsters');
                    }
                }
    ...
    public function deleteMonster() {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if ($stmt->execute([$id])) {
                // Supprimer l'image associée
                $this->deleteOldImage($image, 'monsters');
